...
if ! grep -q "epul.local" /etc/dnsmasq.conf; then
    echo "address=/epul.local/192.168.104.96" | sudo tee -a /etc/dnsmasq.conf
fi
sudo systemctl restart dnsmasq

echo "✅ Semua selesai! Coba akses http://epul.local di browser.Jangan Lupa Ubah DNS Menjadi Ip Ubuntu"
